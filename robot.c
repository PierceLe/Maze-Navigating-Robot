...
void setup_robot(struct Robot *robot){
    ...
    // robot->y = 40;
    // robot->true_x = 10;
    // robot->true_y = OVERALL_WINDOW_HEIGHT/2 -40;
...
int a = 1; //declare a variable to check which is the first time
void robotAutoMotorMove(struct Robot * robot, int front_centre_sensor, int left_sensor, int right_sensor) {
    ...
    if (front_centre_sensor == 0 && left_sensor == 0 && right_sensor == 0){ 
        // this is the situation that every sensors of the robot can not detect anything at the same time. Then we will make the robot walk in an extremely large circle, following the right side until the right sensor detects it.
        if (robot->currentSpeed < 4)
            robot->direction = UP;
        // The first time we will make robot walk in an extremely large circle. However, after that time, if robot struggle with this situation again, we just make robot walk in a small circle, following the right side.
        if (a == 1) {
        robot->angle = (robot->angle+DEFAULT_ANGLE_CHANGE / 6)%360;
        } 
        else {
            robot->angle = (robot->angle+DEFAULT_ANGLE_CHANGE / 2)%360;
        }

    }
    // In another situation, at lease one of three sensor detect the wall. The robot will follow the right side of the maze to get to the destination.
    else{
        if (front_centre_sensor >= 0 && front_centre_sensor <= 1) {
            if (right_sensor < 1 || left_sensor > 3){ 
